...
<?php
	if ( function_exists( 'get_option_tree') ) {
		$apex_folioentrynum = get_option_tree( 'value_portfolioentrynumber' );
		$apex_foliodate = get_option_tree( 'value_portfoliosidebardate' );
		$apex_foliopostinfo = get_option_tree('value_portfoliooverviewpostinfo', '', false, true );
		if(!is_array($apex_foliopostinfo)){ $apex_foliopostinfo = array(); }
	}
...
<div class="eleven columns <?php echo $conoffset ?> row content <?php echo $conorient ?>">
	...
	$args=array(
		'post_type' => $ptype,
		'posts_per_page' => $apex_folioentrynum,
		'paged' => $paged
	);
	$temp = $wp_query; 
	$wp_query = null;
    ...
    <?php while ( $wp_query->have_posts() ) : $wp_query->the_post(); ?>
        ...
        $post_time_daymonthyear = get_post_time('j M Y', true);
        ...
        ?>
            ...
            <div class="eleven columns row alpha blogpost <?php get_post_class(); ?>" id="post-<?php the_ID(); ?>">
                ...
                <?php if($apex_foliodate == "Date Box"){ ?>
                ...
                <?php } ?>
                    ...
                    <div class="postinfo">
                    <?php if (in_array("Date",$apex_foliopostinfo)){ echo $post_time_daymonthyear ?> &nbsp; <span class="divide">|</span> &nbsp; <?php } ?>
					<?php if (in_array("Categories",$apex_foliopostinfo)){ echo $apexlang_in ?> <?php echo $entrycategory ?> &nbsp; <span class="divide">|</span> &nbsp; <?php } ?>
					<?php if (in_array("Author",$apex_foliopostinfo)){ echo $apexlang_by ?> <?php the_author(); ?> &nbsp; <span class="divide">|</span> &nbsp; <?php } ?>
					<?php if (in_array("Comments",$apex_foliopostinfo)){ if ( comments_open() ) : ?><?php comments_popup_link(__('No Comments', 'apex'), __('One Comment', 'apex'), __( '% Comments', 'apex')); ?> &nbsp; <span class="divide">|</span> &nbsp; <?php endif; } ?>
                    </div>
